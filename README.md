...
Re-exporting individual bindings with `export defer { add } from "./math.js"` cannot create a deferred namespace, so the dependency is executed as soon as `add` is imported.
